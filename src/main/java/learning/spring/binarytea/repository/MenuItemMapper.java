...
public interface MenuItemMapper {
    ...
    @Insert("insert into t_menu (name, price, size, create_time, update_time) VALUES (#{name}, #{price}, #{size}, now(), now())")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int save(MenuItem menuItem);

    @Update("update t_menu set name = #{name}, price = #{price}, size = #{size}, update_time = now() where id = #{id}")
    int update(MenuItem menuItem);

    @Select("select * from t_menu where id = #{id}")
    @Results(id = "menuItem", value = {
            @Result(column = "id", property = "id", id = true),
            // size 属性是一个枚举类型，要特殊处理
    ...
    int deleteById(@Param("id") Long id); // Param是指定参数名称，一般不用指定
